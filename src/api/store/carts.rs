...
use crate::application::{
    create_or_get_active_cart, add_cart_item, update_cart_item, remove_cart_item, set_cart_address,
};
...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/carts/active/items/{variant_id}", patch(update_item).delete(remove_item))
...
async fn get_active_cart(
    ...
    Ok(Json(ActiveCartResponse { cart, items, shipping_address }))
}
...
async fn update_item(
    ...
    let mut conn = state.pool.acquire().await?;
    let cart = create_or_get_active_cart::execute(&mut *conn, ctx.customer_id).await?;
    let item = update_cart_item::execute(&mut *conn, ctx.customer_id, cart.id, variant_id, payload).await?;
    Ok(Json(item))
}
...
async fn set_address(
    ...
    let mut conn = state.pool.acquire().await?;
    let cart = create_or_get_active_cart::execute(&mut *conn, ctx.customer_id).await?;
    let addr = set_cart_address::execute(&mut *conn, ctx.customer_id, cart.id, payload.customer_address_id).await?;
    Ok(Json(addr))
}
